...
export const ClassicTemplate: React.FC<TemplateProps> = ({ data }) => {
  ...
  return (
    <div className="w-full h-full bg-white shadow-2xl print:shadow-none min-h-[1123px] max-w-[794px] mx-auto p-8 text-slate-800 print:w-full print:max-w-none text-sm">
      ...
      {/* Header */}
      <div className="flex gap-6 mb-8 border-b-2 border-indigo-600 pb-6 items-center">
        {profile.image && (
      ...
      <div className="space-y-6">
        ...
        {profile.summary && (
            <section>
                <h2 className="text-sm font-bold text-indigo-700 uppercase mb-3 tracking-wider">About Me</h2>
                <div className="text-slate-700 leading-relaxed border-l-2 border-indigo-100 pl-4">
                    {profile.summary}
                </div>
            </section>
        )}
        ...
        {education.length > 0 && (
            <section>
                <h2 className="text-sm font-bold text-indigo-700 uppercase mb-4 tracking-wider border-b border-slate-100 pb-2">Education and Training</h2>
        ...
        {experience.length > 0 && (
             <section>
                <h2 className="text-sm font-bold text-indigo-700 uppercase mb-4 tracking-wider border-b border-slate-100 pb-2">Work Experience</h2>
                <div className="space-y-6">
                    {experience.map(exp => (
        ...
        <div className="space-y-6">
            ...
            {languages && languages.length > 0 && (
                <section>
                    <h2 className="text-sm font-bold text-indigo-700 uppercase mb-4 tracking-wider border-b border-slate-100 pb-2">Language Skills</h2>
            ...
            {/* Skills */}
             {skills.length > 0 && (
                <section>
                    <h2 className="text-sm font-bold text-indigo-700 uppercase mb-4 tracking-wider border-b border-slate-100 pb-2">Skills</h2>
        ...
        {custom && custom.map(section => (
            <section key={section.id}>
                <h2 className="text-sm font-bold text-indigo-700 uppercase mb-3 tracking-wider border-b border-slate-100 pb-2">{section.title}</h2>
                {section.type === 'list' ? (
                   <ul className="list-disc pl-5 text-slate-700 space-y-1 border-l-2 border-indigo-100 pl-6">
                      {section.items.map((item, idx) => (
                        <li key={idx}>{item}</li>
                      ))}
                   </ul>
                ) : (
                  <div className="text-slate-700 leading-relaxed border-l-2 border-indigo-100 pl-4 whitespace-pre-line">
                      {section.description}
                  </div>
                )}
            </section>
